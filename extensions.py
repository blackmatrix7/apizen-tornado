...
from tornsql.session import create_db, new_db, create_sqlite_engine
...
# 数据库配置
demo_engine = create_sqlite_engine(connect_str=current_config.DEMO_DB_CONNECT)
demo_db = create_db(demo_engine)
new_db('demo', demo_engine, demo_db)

logs_engine = create_sqlite_engine(connect_str=current_config.LOGS_DB_CONNECT)
